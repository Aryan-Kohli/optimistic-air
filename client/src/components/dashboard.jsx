...
export default function dashboard(props) {
  ...
  useEffect(() => {
    async function fetchdata() {
      ...
      const userdoc = Cookies.get("user");
      if (!token || !userdoc) {
        setOut("/");
        return <Navigate to="/login" replace={true} />;
      }
      const parsed = JSON.parse(token);
      const parsed_user = JSON.parse(userdoc);
      setUser(parsed_user);
      setToken(parsed);
    }
    fetchdata();
  ...
  useEffect(() => {
    ...
    if (user) {
      try {
        const data = user.prevresults;
        console.log("previous results is : ", data);
              ...
              <section className="container" id="analytics">
                <div className="analysis">
                  ...
                  {prevResults.length == 0 ? (
                    ...
                    <div
                      className="accordion test-result"
                      id="accordionExample"
                    >
                      <ul>
